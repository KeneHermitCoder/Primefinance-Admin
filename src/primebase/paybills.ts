...
export class PayBillsAPI {
    ...
    async billerList(categoryName: string): Promise<DataResponse> {
        const response = await httpClient(`${this.URL}/biller-list?categoryName=${categoryName}`, "GET", {}, true);

        return DataHttpResponse(response);
    }

    async billerCategories() : Promise<DataResponse> {
        const response = await httpClient(`${this.URL}/biller-categories`, "GET", {}, true);
